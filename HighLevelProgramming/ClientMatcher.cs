...
namespace HighLevelProgramming
{
    public class ClientMatcher
    {
        ...
        public VideoStore ClosestRecommendedStore(int clientId)
        {
            ...
            if (store == null)
                throw new ArgumentException($"No store found for client ${client.FirstName}");
            return store;
        }
        ...
        public VideoStore RecommendedStoreWithShortestWait(int clientId)
        {
            ...
            Array.Sort(compatibleStores, (s1, s2) => s1.AveragePlaceInLine(client).CompareTo(s2.AveragePlaceInLine(client)));
            
            //Return first item in array, if it exists
            VideoStore store = compatibleStores[0];
            if (store == null)
                throw new ArgumentException($"No compatible store found for client ${client.FirstName}");
            return store;
        }
    }
}